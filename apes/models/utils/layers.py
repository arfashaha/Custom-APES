...
class Embedding(nn.Module):
    def __init__(self, input_dim=3):  # default 3 (XYZ)
        super(Embedding, self).__init__()
        self.K = 32
        self.group_type = 'center_diff'
        self.input_dim = input_dim

        # Dynamically use input_dim for the first conv
        self.conv1 = nn.Sequential(
            nn.Conv2d(12, 128, 1, bias=False),  # because grouped diff doubles channels
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2)
        )
    ...
    def forward(self, x):
        x_list = []
        x = ops.group(x, self.K, self.group_type)  # output: (B, 2*C, N, K)
        x = self.conv1(x)
        x = self.conv2(x)
        ...
        x = torch.cat(x_list, dim=1)  # (B, 128, N)
        return x
